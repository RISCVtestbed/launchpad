...
static int get_number_active_cores(struct launchpad_configuration*);
...
void poll_for_uart_across_cores(struct launchpad_configuration * config, struct device_drivers active_device_drivers) {
  int num_active_cores=get_number_active_cores(config);
  ...
  if (num_active_cores > 1) {
    ...
    output_buffers=(char**) malloc(sizeof(char*) * config->total_cores_on_device);
    output_buffer_locals=(unsigned int*) malloc(sizeof(unsigned int) * config->total_cores_on_device);
...
static int get_number_active_cores(struct launchpad_configuration * config) {
